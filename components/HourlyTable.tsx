...
export const HourlyTable: React.FC<HourlyTableProps> = ({ 
  ...
  const keys = useMemo(() => {
    ...
    const updatedKey = allKeys.find(k => k.toLowerCase().includes('updated') || k.toLowerCase().includes('cập nhật')) || '';
    return allKeys.filter(k => k !== updatedKey);
  }, [data]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2">
                 ...
                 <span className={`text-[10px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded font-bold transition-opacity duration-300 ${showMetadata ? 'opacity-100' : 'opacity-0 hidden'}`}>
                    {processedData.length} MỐC
                 </span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
          <div className={`flex items-center transition-all duration-300 ${isSearchExpanded ? 'w-40 md:w-64 bg-slate-50 ring-1 ring-slate-200' : 'w-10'} rounded-xl overflow-hidden`}>
            <button 
              ...
              className={`p-2 transition-colors ${isSearchExpanded ? 'text-blue-600' : 'text-slate-500 hover:bg-slate-50'} rounded-xl`}
              title="Tìm kiếm"
            >
          ...
          {onRefresh && <button onClick={onRefresh} className="p-2 text-slate-500 hover:bg-slate-50 hover:text-blue-600 rounded-xl border border-slate-100 transition-colors"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg></button>}
        </div>
      </div>
